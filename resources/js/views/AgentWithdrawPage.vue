    ...
    <section class="home-section">
        <div class="center">
            <div class="top-nav-inivte">
                ...
                <a href="javascript:void(0)">Agent Withdraw</a>
                <a @click="$router.push({ name: 'Invite' })"><i class="fa-solid fa-arrow-right-from-bracket" style="color: #fff;"></i> </a>
            </div>
            <div class="withdraw-boxes1">
                <div class="withdraw-boxes1-heading">
                    ...
                    <p class="withdraw-boxes1-heading-amount">&#8377; {{ this.bonus }}</p>
                </div>
                ...
                <div class="withdraw-boxes1-amount-box">
                    ...
                    <p class="max-price">
                        Maximum: &#8377; {{ this.bonus }} | Minimum: &#8377; 300
                    </p>
                    ...
                    <div class="Withdraw-method-box-amount-box">
                        ...
                        <div class="Withdraw-method-box-amount-box-add-upi-btn">
                            ...
                            <div class="form-check">
                                <input class="form-check-input bank" type="radio" name="bank" id="bank" :value="rows.id"
                                    required=""  :checked="this.user.bank == rows.id">
                            </div>
                        </div>
                    </div>
                </label>
        ...
        async withdraw() {
            if (this.allowWithdrow()) {
                if (this.user.amount >= 300) {
                    ...
                    data.append('amount',this.user.amount);
                    data.append('bank',this.user.bank);
                    const res = await this.callApi('post', 'api/user/v1/agent-withdraw', data);
                    if (res.status === 200) {
                        this.salert(res.data.msg);
                        this.$router.push({ name: 'AgentWithdraw' });
                    } else {
                        if (res.status === 401) {
                            this.logout();
