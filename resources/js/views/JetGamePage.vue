        ...
        <div class="center avitor-game-center">
            ...
            <div class="jet-bet">
                ...
                <div class="d-flex justify-content-around my-2 align-content-center">
                    ...
                    <div class="addbtn" @click="add(200)">+200</div>
                    <div class="addbtn" @click="add(500)">+500</div>
                </div>
                <div class="row my-2">
                ...
                <div class="bet-btn-box" id="waiting_btn">
                    <button type="button" @click="joinwait()">Join Bet For Next</button>
                </div>
                <!-- <div class="bet-btn-box" id="join_btn" style="display:none;">
                        <button type="button" @click="joinbet()">Join BET</button>
                    </div>
                    <div class="bet-btn-box" id="cancle_btn" :style="{ display: this.betwaiting ? 'block' : 'none' }" style="display:none;">
                        <button type="button" @click="canclebet();">Cancle Bet</button>
                    </div>
                    <div class="bet-btn-box" id="cashout_btn" :style="{ display: this.bet && this.betwaiting ? 'block' : 'none' }">
                        <button type="button" @click="makecashout(this.multiple, this.betamount, this.username);">Cash
                            Out</button>
                    </div>
                    <div class="bet-btn-box" id="waiting_btn"  :style="{ display: this.flybol && this.bet == false ? 'block' : 'none' }" >
                        <button type="button" @click="joinwait()">Join Next BET</button>
                    </div> -->
            </div>
            ...
            <div class="modal winner-modal fade" id="c_resulPopup" tabindex="-1" aria-labelledby="exampleModalLabel"
                aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-sm">
                    <div class="modal-content">
                        <div class="modal-body">
                            <div class="winner-count">he</div>
                            <h4>hello</h4>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    mounted: function () {
        this.socket = io('https://YUVIWIN.up.railway.app', {
    ...
    methods: {
        ...
        joinbet() {
            if (this.balance > this.prebet && this.prebet >= 10) {
                ...
                this.betamount = this.prebet;
                this.isbet = true;
                // console.log('bet', this.betwaiting, 'hello', this.betamount);
            } else {
        ...
        add(amount) {
            if (this.balance >= this.prebet + amount) {
                this.prebet = this.prebet + amount;
            } else {
                // console.log("hello")
            }
        },
        ...
        initializeGame() {
            const width = $('#jetbox').width();
            const height = $('#jetbox').height();
        ...
        flyjet() {
            ...
            jetImage.onload = () => {
                ...
                loadingGroup.add(jet);
                const fireAnimation = this.createFireAnimation(jet);
                ...
                stage.add(layer);
                this.startJetAnimation(layer, jet, stage, fireAnimation, curvedLine, graphGroup);
            };
            const anim = new Konva.Animation(() => { }, layer);
            anim.start();
        },
        drawGraph(graphGroup, currentLabelIndex, numTicksY, numTicksX, width, height) {
            ...
            if (this.ychanges > 80) {
                ...
                this.ychanges = 40 * (this.multiple - (Math.trunc(this.multiple)));
            }
            for (let i = numTicksY; i >= 0; i--) {
                var tickY = ((numTicksY - i) / numTicksY) * (height - 10);
                if (this.currentLabelIndex > 3) {
                    tickY = tickY + this.ychanges;
                }
                // console.log(this.ychanges);
                ...
                const label = new Konva.Text({
                    x: width - 30,
                    y: tickY,
                    text: currentLabelIndex.toString(),
                    fontSize: 12,
                    fill: 'whitesmoke',
                });
        ...
        showRealTimeUsers(users, jet, graphGroup) {
            users.forEach(user => {
        ...
        startJetAnimation(layer, jet, stage, fireAnimation, curvedLine, graphGroup) {
            const width = $('#jetbox').width();
            const height = $('#jetbox').height();
            const buttonGroup = new Konva.Group();
            const square = new Konva.Group();
            const squareRect = new Konva.Rect();
            const crshbtn = new Konva.Rect();
            const crashpointtext = new Konva.Text();
            const cashOutButtonGroup = new Konva.Group();
            const cashButton = new Konva.Rect();
            const cashButtonText = new Konva.Text();
            cashOutButtonGroup.on('click', () => {
                this.cashOut();
            });
            const anim = new Konva.Animation((frame) => {
                const speed = frame.timeDiff / 1000;
                let newX = jet.x();
                let newY = jet.y();
                if (jet.x() > width - 80) {
                    newX = jet.x();
                ...
                // console.log('y: ',jet.y(),'new Y:',jet.y() - (((height - 70) * 1 / 3)/3)*.1)
                ...
                if (this.currentLabelIndex < this.multiple) {
                    this.currentLabelIndex = Math.trunc(this.multiple);
                }
                this.drawGraph(graphGroup, this.currentLabelIndex, 3, 10, width, height);
        ...
        crash() {
            ...
            stage.add(bgLayer);
            stage.add(layer);
            this.jetCrashed(stage, layer, width, height);
        },
        jetCrashed(stage, layer, width, height) {
            const blastGroup = new Konva.Group({
                x: width / 2,
                y: height / 2,
            });
