    ...
    <section class="home-section">
        <div class="center fast-parity-game" style="min-height: 100vh!important;">
            <div class="top-nav-inivte" style="background:#fff;">
                <a @click="$router.push({ name: 'Home' })"><i class="fa-solid fa-home"
                        style="color: var(--bg-color);"></i></a>
                <a href="javascript:void(0)" style="color: var(--bg-color);">Fast-Parity</a>
                <a  @click="this.telegram()" href="javascript:void(0)" style="color: var(--bg-color);"><img src="../assets/img/telegram-icon.png"></a>
            </div>
            <div class="game-details-fp">
                ...
                <div class="btn-box" v-if="allowedbet == true">
                          <div class="item left" id="joinred" data-bs-toggle="offcanvas" data-bs-target="#offcanvasBottom"
                        aria-controls="offcanvasBottom" @click="join('Big')">
                        <p style="text-align: center;color: #fff;">Big</p>
                    </div>
                    <div class="item right" id="joinred" data-bs-toggle="offcanvas" data-bs-target="#offcanvasBottom"
            ...
            <div class="bet-filter-box fast-bg">
                <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist" style="border: none;">
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="pills-home-tab" type="button" role="tab"
                            @click="$router.push({ name: 'GameHistory', params: { game: 'fast-parity' } })"
                ...
                <div class="tab-content" id="pills-tabContent">
                    <div class="tab-pane fade show active" id="betFilter" role="tabpanel" aria-labelledby="betFilter-tab">
                        <div class="betFilter-table-box">
                            <table class="avitor-game1-table game-table fast-parity">
                                <tr class="heading">
                                    <th>Period</th>
                                    <th>user</th>
                                    <th>select</th>
            ...
            <div class="wlbox " id="resultPopup">
                <div v-if="this.userlastresult.issuccess == true" class="wlbox-main">
                    ...
                    <div class="result" :class="getResultClass(this.userlastresult.last.clo)">
                        <div class="option" style="display: block;" :class="getResultClass(this.userlastresult.last.clo)">
                            <div class="violet " v-show="this.userlastresult.last.res1 === 'violet'"></div>

                            <div class="option option2" style="position: relative;">{{ this.userlastresult.last.ans }} {{ this.userlastresult.last.ans <= 4 ? 'S' : 'B' }}</div>
                        </div>
                    </div>
                    ...
                    <div v-if="this.userlastresult.issuccess == true" class="yourresult">
                        <div class="mainr">
                            <div class="row">
                            ...
                            <div v-for=" rows  in  userlastresult.ans " :key="rows.id" :rowsrec="rows" class="row">
                                <div v-if="rows.res === 'success'" class="row">
                                <div class="col-4">
                                    <div class="select center-content" :class="getcolor(rows.ans)">{{ short(rows.ans) }}</div>
                                </div>
                                <div class="col-4  text-center">-&#8377; {{ getfeefastParity(rows.amount, rows.ans,
                                            rows.color2) }}</div>
            ...
            <div v-if="allowedbet == true" class="fast-parity-popup">
...
.bet-filter-box.game-record .game-table.fast-parity tr th,
.game-table tr td {
    color: #000 !important;
}
...
export default {
    ...
    async placeBet() {
        ...
        countdown() {
            ...
            if (30 == distance || 29 == distance ||  59 == distance ||  60 == distance) {
                this.fechresultrec().then(() => {
                    if(this.userlastresult.issuccess == true){
        ...
        async getBets() {
            const data = new FormData();
            data.append('period', this.nexttimestamp)
            const res = await this.callApi('post', '/api/user/v1/fast-parity/bets', data);
            if (res.status === 200) {
