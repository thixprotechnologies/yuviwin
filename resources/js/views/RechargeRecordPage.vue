    ...
    <section class="home-section">
        <div class="center">
            <div class="top-nav-inivte">
                <a href="javascript:void(0)">Recharge Record</a>
                <router-link :to="{ name: 'Recharge' }"><i class="fa-solid fa-arrow-right-from-bracket"></i></router-link>
            </div>
            <div class="income" style="margin-top:6rem;">
                <details v-for="rows  in  recharge " :key="rows.id" :rowsrec="rows">
                    <summary>
                        <div>
                            <h3>
                                <strong>Recharge</strong>
                                <small :style="{ color: getStatusColor(rows.status) }" >{{ rows.status_str }}</small>
                            </h3>
                            <span>&#8377; {{ rows.recharge }}</span>
                        </div>
                    </summary>
                    <div>
                        <dl>
                            ...
                            <div>
                                <router-link class="ncmpbtn" :to="{ name: 'ComplaneCreate', query: { id: rows.id, type: 'Recharge' } }">Complain</router-link>
                            </div>
                        </dl>
                    </div>
                </details>
...
export default {
    ...
    methods: {
        ...
        async rechargeRecord() {
            const res = await this.callApi('post', '/api/user/v1/payment/records');
            if (res.status === 200) {
