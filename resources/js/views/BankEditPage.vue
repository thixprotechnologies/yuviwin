    ...
    <section class="home-section">
        <div class="center">
            <div class="top-nav-inivte">
                ...
                <a @click="$router.push({ name: 'Withdraw' })"><i class="fa-solid fa-arrow-right-from-bracket"
                        style="color: #fff;"></i> </a>
            </div>
            <!-- End top nav -->
            ...
            <div class="bankAccount-form">
                <div id="form">
                    <div class="inputBox">
                        ...
                        <input type="text" id="name" v-model="user.name" ref="name" placeholder="Ex. State Bank of India"
                            autocomplete="off">
                    </div>
                    <div class="inputBox">
                ...
                <div class="bankAccount-submit-btns">
                    <button class="cancel-btn" type="button" @click="deleteBank()">Delete</button>
                    <button :class="{ 'addBank-btn': isBankDetailsValid() }"
                        @click="isBankDetailsValid() ? addBankdetails() : null" type="button">Update Bank</button>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        async getBankdetails() {
            ...
            if (res.status === 200) {
                if (res.data.back) {
                    ...
                    this.user.bankname = res.data.bank.bank_name;
                    this.user.name = res.data.bank.name;
                    this.user.ifscCode = res.data.bank.ifsc_code;
                    this.user.accountNumber = res.data.bank.acc_number;
                }
            } else {
                if (res.status === 401) {
                    this.logout();
        ...
        async deleteBank() {
            var formdata = new FormData();
            formdata.append('id', this.user.id);
            const res = await this.callApi('post', '/api/user/v1/bank-delete', formdata);
            if (res.status === 200) {
                this.salert(res.data.msg);
                this.$router.push({ name: 'Withdraw' });
            } else {
